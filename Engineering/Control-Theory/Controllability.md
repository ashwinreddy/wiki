...
Controllable
: A system is controllable if for every $x_{\text{target}} \in \RR^n$, there exists a sequence of inputs $\vec{u}_i$ from any initial state $\vec{x}(0)$ to get to $\vec{x}_{\text{target}}$.
