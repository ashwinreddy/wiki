...
$$
\log p_{\pmb{\theta}}\left(\mathbf{x}^{(1)}, \dots, \mathbf{x}^{(N)} \right) = D_{KL}\left(q_{\pmb{\phi}}\left(\mathbf{z} \mid \mathbf{x}^{(i)}\right) \parallel p_{\pmb{\theta}}\left(\mathbf{z} \mid \mathbf{x}^{(i)}\right) \right)
$$
