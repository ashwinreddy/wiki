...
$$
\log p_{\pmb{\theta}}\left(\mathbf{x}^{(i)} \right) = D_{KL}\left(q_{\pmb{\phi}}\left(\mathbf{z} \mid \mathbf{x}^{(i)}\right) \middle\parallel p_{\pmb{\theta}}\left(\mathbf{z} \mid \mathbf{x}^{(i)}\right) \right) + \mathcal{L}\left(\pmb{\theta}, \pmb{\phi}; \mathbf{x}^{(i)}\right)
$$
