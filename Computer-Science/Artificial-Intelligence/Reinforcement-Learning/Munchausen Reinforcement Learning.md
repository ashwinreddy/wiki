...
\begin{equation}
\hat{q}\_{\mathrm{s-dqn}}(r_t, s_{t+1}) = r_t + \gamma \sum_{a' \in \mathcal{A}} \pi_{\bar\theta}(a' \mid s_{t+1})\Big(q_{\bar\theta}(s_{t+1}, a') \color{blue} -\tau \ln \pi_{\bar\theta}(a' \mid s_{t+1})  \color{black} \Big) \text{ with } \pi_{\bar\theta} = \mathsf{softmax}\left(q_{\bar\theta} / \tau\right)
\end{equation}
