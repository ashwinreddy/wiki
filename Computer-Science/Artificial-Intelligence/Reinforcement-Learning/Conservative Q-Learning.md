...
As an offline algorithm, we have access to a dataset $\mathcal{D}$. We write the action-conditional for the dataset as $\pi_\beta(a \mid s)$ and the marginal state distribution as $d^{\pi_\beta}(s)$.
