...
The [[kernel method]] here comes by considering a [[dual problem|duality]]. In dual ridge regression, we consider $\mathbf{w} = \mathbf{X}^\mathsf{T}\pmb{\alpha}$ where $\alpha$ is a vector of dual weights. The optimal solution is then
