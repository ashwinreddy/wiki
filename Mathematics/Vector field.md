A **vector field** is a [[tensor field]] that assigns [[tangent vectors|tangent vector]] to points on a [[manifold]] $M$. Say, for example, that $X$ is a vector field on $M$. Then, for every point $p$ on the manifold,
