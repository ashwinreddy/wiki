The **circle group** $\mathbb{T}$ is an abelian [[Lie group]] where the elements are unit circles and the operation is rotation. Its elements are the set of points on a unit circle with group action as rotation. Because of its simplicity, it can be represented in a variety of equivalent ways.
