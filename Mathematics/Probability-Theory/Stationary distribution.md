...
\begin{equation}
\label{eq:be}
\pi_j = \sum_{i \in \mathcal{X}} \pi_i P_{ij} \implies \pi = \pi P. \tag{Balance Equations}
\end{equation}
...
Notice that if \ref{eq:dbe} is true, then \ref{eq:be} must be true. Intuitively, here is what it means:
...
An irreducible positive recurrent aperiodic chain converges to $\pi$.
