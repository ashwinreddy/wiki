...
Riemann sums are basically a specification with concrete implementation details of the integral. Fundamentally, the Riemann sum tries to compute
...
The Stieltjes integral basically allows for computations w.r.t. something other than a $\diff{x}$, and it shows up quite often. First, we start with a function $\alpha(x)$.
...
\begin{equation}
\int_C{\mathcal{O}\left(\vec{r}\right)\,\mathrm{d}s} = \defint{a}{b}{\mathcal{O}\left(\vec{r}(t)\right)\left|\dot{\vec{r}}\right|}{t}
\end{equation}
