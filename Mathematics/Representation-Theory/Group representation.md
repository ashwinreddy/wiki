A **group representation** is a [[group homomorphism]] for a [[group]] $G$ into a [[general linear group]] $\rho: G \to \operatorname{GL}(V)$. It is a way of describing the way a group transforms objects abstractly using a matrix. While the representation is technically a mapping, people also use it to refer to the vector space it acts over too.
